    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (bookingsData.status === 'success') {
          ...
          const formattedBookings = bookingsData.data.slice(0, 5).map((b: any) => ({
            id: b.id.toString(),
        ...
        if (reportsData.status === 'success') {
          const totalRev = reportsData.data.reduce((sum: number, item: any) => sum + Number(item.total_revenue), 0);
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            ...
            <div className="xl:col-span-2 flex flex-col">
              <Card noPadding className="flex-1 shadow-sm border-slate-200">
                <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-white/50 backdrop-blur-sm">
                  ...
                  <a href="/admin/bookings" className="inline-flex items-center gap-1 text-sm font-semibold text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 px-3 py-1.5 rounded-lg transition-all duration-200 ease-out">
            ...
            <div className="xl:col-span-1 flex flex-col">
              <Card className="flex-1 p-6 shadow-sm border-slate-200 bg-white relative overflow-hidden group cursor-default">
                ...
                <h3 className="text-lg font-bold text-slate-900 relative z-10 mb-6">Info Akses Cepat</h3>
                ...
                <div className="flex-1 bg-slate-50/80 rounded-2xl border border-slate-200/60 flex flex-col items-center justify-center min-h-[320px] p-8 text-center relative z-10">
                  ...
                  <p className="text-slate-900 font-bold mb-2">Lihat Analitik Lengkap</p>
                  <p className="text-sm text-slate-500 font-medium leading-relaxed max-w-[200px] mb-6">
                    Buka halaman laporan untuk melihat tren pendapatan bulanan dan performa lapangan.
                  </p>
                  <a href="/admin/reports" className="inline-flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-6 rounded-xl transition-colors w-full">
                    Buka Laporan
                  </a>
                </div>
              </Card>
